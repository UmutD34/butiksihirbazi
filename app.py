...
if st.session_state.selected_product is None:
    ...
    for idx, p in enumerate(display_items):
        with cols[idx % 4]:
            ...
            else: st.markdown("<div style='height:200px; display:flex; align-items:center; justify-content:center; color:#ccc;'>Görsel Yok</div>", unsafe_allow_html=True)
...
# MOD 2: DETAY SAYFASI
    ...
    with col_right:
        ...
        if p.get('price'): st.markdown(f"<h3 style='color: #86868b; margin-top: 0;'>{p['price']}</h3>", unsafe_allow_html=True)
        st.markdown("<br>", unsafe_allow_html=True)
